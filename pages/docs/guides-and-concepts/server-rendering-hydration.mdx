...
배경 지식을 원하시면 [Prefetching & Router Integration](/docs/guides-and-concepts/prefetching-router-integration) 가이드를 참조하세요. [Performance & Request Waterfalls](/docs/api-references/request-waterfalls) 가이드를 먼저 읽어보는 것도 좋습니다.
...
Prefetching 가이드에서는 [의존 쿼리 미리 가져오기](/docs/guides-and-concepts/prefetching-router-integration#dependent-queries--code-splitting)를 배웠지만, 프레임워크 로더에서 이를 어떻게 처리할까요? 다음은 [의존 쿼리 가이드](/docs/guides-and-concepts/dependent-queries)에서 가져온 코드입니다:
